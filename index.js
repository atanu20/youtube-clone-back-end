...
router.post("/profile",async (req,res)=>{
    ...
    try{
        ...
        let targetFile = req.files.profileimg;
    ...
    // console.log(targetFile)
        ...
        let uploadDir = path.join(__dirname, '../client','public','profile', num+"_"+targetFile.name);
        
    
        let imgList = ['.png','.jpg','.jpeg','.webp','.svg'];

        if(!imgList.includes(extName)  ){
      
            return res.status(500).json({submit:false,msg:"Only jpg ,jpeg ,webp ,svg and png"})
        }
        
    
        if(targetFile.size > 4000000 ){
           
           return res.status(500).json({submit:false,msg:"File should be less then 4 MB "})
        }
        targetFile.mv(uploadDir, async (err) => {
            if (err)
            {
                ...
                const usedet=new profileTable({channel:req.body.channel,profile_image:imgname,user_id:req.body.user_id})
                const resu= await usedet.save();
...
router.get('/myalldata/:myid',async(req,res)=>{
    try{
       ...
       if(profiles)
       {
        ...
        res.send({name:userdetails.name,profile_image:"pro.png"})
       }
