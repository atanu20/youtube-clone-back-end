...
router.post('/addvideo', async (req, res) => {
  try {
    // videoTable
    const usedet = new videoTable({
      channel: req.body.channel,
      profile_image: req.body.profile_image,
      title: req.body.title,
      channelthumb: req.body.urlthumb,
      channelvideo: req.body.url,
      tags: req.body.tag,
...
router.post('/subscribe', async (req, res) => {
  try {
    ...
    // console.log(post)
    const updatedPost = await profileTable.findByIdAndUpdate(
      { _id: post._id },
      { subscriber: post.subscriber + 1 },
      { new: true }
    );

    const usedet = new subTable({
      channel: req.body.channel,
      uid: req.body.userid,
    });
    const resu = await usedet.save();
...
router.post('/dolike', async (req, res) => {
  try {
    ...
    const updatedPost = await videoTable.findByIdAndUpdate(
      { _id: videoid },
      { likes: post.likes + 1 },
      { new: true }
    );
...
router.post('/savehistory', async (req, res) => {
  try {
    ...
    // console.log(post)
    const usedet = new hisTable({
      channel: post.channel,
      thumbimg: post.channelthumb,
      title: post.title,
      vid: vid,
      uid: req.body.uid,
    });
